...
<body>
    ...
    <tr><td>
			...
			$query  = "SELECT * FROM asignacion LEFT JOIN grupo ON asignacion.Grupo_Id = grupo.Grupo_Id LEFT JOIN materia ON grupo.Materia_Id = materia.Materia_Id LEFT JOIN carrera_materia ON carrera_materia.Materia_Id = materia.Materia_Id LEFT JOIN profesor ON asignacion.Profesor_Id = profesor.Profesor_Id LEFT JOIN lugar ON asignacion.Lugar_Id = lugar.Lugar_Id LEFT JOIN horarios_disp ON asignacion.Horario_Disp_Id = horarios_disp.Horario_Disp_Id LEFT JOIN dia ON horarios_disp.Dia_Id = dia.Dia_Id WHERE carrera_materia.Carrera_Id = '".$_GET["carr"]."' AND carrera_materia.Semestre = ".$_GET["sem"];
			
			$result = mysql_query($query);
			$count=mysql_num_rows($result);
			if($count>=1){
			echo "<table onMouseover=\"changeto(event, 'lightblue')\" onMouseout=\"changeback(event, 'white')\" align='center' width='800' border='0'>";
			echo "<tr id=\"ignore\"><td><b>ID</b></td><td><b>MATERIA</b></td><td><b>PROFESOR</b></td><td><b>LUGAR</b></td><td><b>DÍA</b></td><td><b>HORA</b></td></tr>";			
			while ($row = @mysql_fetch_array($result, MYSQL_ASSOC)) {
				echo "<tr><td>".@$row['Grupo_Id']."</td><td>".@$row['Materia_Nombre']."</td><td>".@$row['Profesor_Nombre']."</td><td>".@$row['Codigo']."</td><td>".@$row['Dia_Nombre']."</td><td>".$stack[@$row['Hora_Inicio']-1]." - ".$stack[@$row['Hora_Fin']-1]."</td></tr>";
			}
			echo "</table><br />";
			} else {
			echo "<br><b>No existen grupos registrados para el semestre de la carrera.</b>";
			}
