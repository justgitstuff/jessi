...
<body>
    ...
    <tr><td><b><font color="#000000">[ REPORTES - PROFESORES > GRUPOS ]</font></b></td></tr>
    <tr height="50"><td></td></tr>
    <tr><td>
	</table>
            <?php
			...
			echo "<center><div style='font-size:15px; font-weight:bold'>Profesor: &nbsp;".$row_aux['Profesor_Nombre']."</div></center><br/><br/>";
			...
			$query  = "SELECT * FROM asignacion LEFT JOIN lugar ON asignacion.Lugar_Id = lugar.Lugar_Id LEFT JOIN horarios_disp ON asignacion.Horario_Disp_Id = horarios_disp.Horario_Disp_Id LEFT JOIN dia ON horarios_disp.Dia_Id = dia.Dia_Id LEFT JOIN grupo ON asignacion.Grupo_Id = grupo.Grupo_Id LEFT JOIN materia ON grupo.Materia_Id = materia.Materia_Id  WHERE asignacion.Profesor_Id = '".$_GET["id"]."'";
			$result = mysql_query($query);
			$count=mysql_num_rows($result);
			if($count>=1){
			echo "<table onMouseover=\"changeto(event, 'lightblue')\" onMouseout=\"changeback(event, 'white')\"  align='center' width='600' border='0'>";
			echo "<tr id=\"ignore\"><td><b>ID</b></td><td><b>MATERIA</b></td><td><b>LUGAR</b></td><td><b>DÍA</b></td><td><b>HORA</b></td></tr>";			
			while ($row = @mysql_fetch_array($result, MYSQL_ASSOC)) {
				echo "<tr><td>".@$row['Grupo_Id']."</td><td>".@$row['Materia_Nombre']."</td><td>".@$row['Codigo']."</td><td>".@$row['Dia_Nombre']."</td><td>".$stack[@$row['Hora_Inicio']-1]." - ".$stack[@$row['Hora_Fin']-1]."</td></tr>";
			}
			echo "</table><br />";
			} else {
    ...
    <tr height="20"><td></td></tr>
    <tr><td colspan="2"><center><a href="profesores.php">REGRESAR</a></center></td></tr>
